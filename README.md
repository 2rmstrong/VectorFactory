...
通过将 A 股市场的微观行为转化为高维特征向量，实现了从数据清洗、信号挖掘到 T+1 实盘回测的闭环。硬件要求非常友好。

数据方面，也只需要准备门槛最低的 **tushare API** 。
...
├── core/                  # 核心算力层：统一的向量化撮合底座
├── data_pipeline/         # 数据流管道：外部 API 接入与 DuckDB 增量更新层
├── strategies/            # 向量工厂：策略信号提取脚本
├── backtests/             # 实验沙盒：策略的历史回测与资金曲线绘制
└── docs/                  # 存档：储存回测图片
```

## 🚀 第一批策略矩阵 (Strategies)

本库首批开源了 9 套涵盖不同市场失效模式的量化模型。

“万变不离其宗”。理论上，任何策略都可通过不断修改和优化这些参数和资金管理模型做到。

所有策略逻辑均已完成 A 股特有的涨跌停流动性限制与非对称交易成本适配。

## 💡 核心 Alpha 提示 (基于 10 年以上的数据回测)

经过 10 年以上数据样本回测验证，在当前 A 股市场的微观结构下，本矩阵中有效性最强、夏普比率最高的两套模型为：

SM-05 (Fama-French 小市值底座)：在 A 股，“微小市值+低估值”因子的超额收益（Alpha）极其显著且具备长期持续性，是截面选股的最强底座因子之一。有效性持续已超过 10年。
![SM-05 回测曲线](docs/ff_nav_20150105_20260320.png)

SM-07 (Pairs Trading 协整配对)：利用强相关资产的均值回归特性，在震荡市与熊市中均表现出极强的抗跌能力，贡献了极为稀缺的“市场中性 Alpha”。

## 📡 策略信号说明 (Signal Engine)

工厂内的每一套策略，都是对市场特定特征的数学剥离。以下是第一批 **4 大类**、共 **9 个模块**的核心触发逻辑：
...
但由于涉及全市场标的的截面两两校验，计算压力极大。项目底层算子已针对 **GPU 加速** 优化，推荐在具备 **CUDA 环境** 的终端上运行，可以实现分钟级别的全场扫描。
...
动能衰竭探测器。寻找价格创出阶段新低，但 RSI 指标未能创新低的背离奇点，确定左侧买点。

#### SM-09 (PEAD 盈余漂移)

对财报公布日的超预期程度进行量化（利用公布当日的市场指标）。当实际利润击穿一致预期时，捕捉市场定价滞后产生的连续向上漂移利润。
...
这里没有保证盈利的圣杯，只有对代码效率和数学逻辑的偏执。

第一批仅涉及 9 个最基础的原型策略，未来会对一些有效的主观策略进行量化开发，部分开源。

开源策略的所有参数和资金模型可自行组合、优化。
